...
import { CheckCircle2, XCircle, ShoppingBag, ShoppingCart as CartIcon, CreditCard } from 'lucide-react';
...
const PaymentResultPage = () => {
  ...
  useEffect(() => {
    // ABA PayWay returns status in query params
      ...
      >
        ...
        <div className={`pt-12 pb-8 px-8 ${isSuccess ? 'bg-gradient-to-b from-green-50 to-white' : 'bg-gradient-to-b from-red-50 to-white'}`}>
          ...
          <p className="text-gray-500 font-medium">
            {isSuccess ? 'Thank you for your purchase. Your transaction is complete.' : 'We could not process your payment at this time. Please try again or use another payment method.'}
          </p>
        </div>
        ...
        <div className="px-8 pb-8">
          ...
          <div className="space-y-3">
            <Button 
              ...
              className={`w-full h-14 text-lg font-bold text-white rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all ${isSuccess ? 'bg-[#005aab] hover:bg-[#004481] shadow-blue-600/20' : 'bg-[#005aab] hover:bg-[#004481] shadow-blue-600/20'}`}
            >
            ...
            {!isSuccess && (
                <Button 
                    ...
                    className="w-full h-14 text-lg font-bold text-gray-700 hover:text-gray-900 hover:bg-gray-100 rounded-xl flex items-center justify-center gap-2 border-2 border-gray-200"
                >
                    <CartIcon className="w-5 h-5" /> View Cart to Retry
                </Button>
            )}
          </div>
        </div>
